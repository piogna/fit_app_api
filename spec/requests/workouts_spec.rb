...
RSpec.describe "Workouts API", :type => :request do
  context "#index" do
    ...
    it "should not contain workout_exercises_attributes" do
      expect(json[0]["workout_exercises_attributes"]).to be_nil
    end
  ...
  context "#show" do
    before(:each) do
      ...
      get "/api/workouts/#{@workout.id}", format: :json
    end

    it "should return the right workout" do
      expect(json["id"]).to eq(@workout.id)
    end

    it "should include workout_exercises_attributes" do
      expect(json["workout_exercises_attributes"]).to_not be_nil
    end

    it "should include ex_sets_attributes" do
      expect(json["workout_exercises_attributes"][0]["ex_sets_attributes"]).to_not be_nil
    end
  end
  context "#create" do
